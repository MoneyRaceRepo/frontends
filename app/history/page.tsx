...
import { HiSparkles, HiCheckCircle, HiLockClosed, HiUserGroup, HiLightBulb, HiExclamationCircle, HiInformationCircle } from "react-icons/hi";
import { RiVipCrownFill, RiVipCrown2Fill, RiToolsFill } from "react-icons/ri";
import { FaInfinity, FaRocket, FaCrown, FaShieldAlt, FaUsers, FaChartLine, FaHeadset, FaStar, FaBolt } from "react-icons/fa";
  ...
  return (
    <DashboardLayout>
      ...
      <div className="mb-6 bg-gradient-to-r from-[#E8D5A8] to-[#D4C5A0] border-2 border-[#D4A84B] rounded-xl p-4 shadow-lg">
        <p className="text-[#4A3000] text-xs font-semibold mb-2 flex items-center gap-2">
      ...
      {/* Plan Cards Grid - Equal Height Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        ...
        <PlanCard
          ...
          features={[
            { label: "Public Room Creation", value: "Unlimited", icon: <FaUsers className="w-4 h-4" /> },
            { label: "Private Room Creation", value: "Unlimited", icon: <HiLockClosed className="w-4 h-4" /> },
            { label: "AI Recommendation", value: "Unlimited", icon: <HiLightBulb className="w-4 h-4" /> },
            { label: "24/7 Priority support", value: "Included", icon: <FaHeadset className="w-4 h-4" /> },
            { label: "Advanced analytics", value: "Included", icon: <FaChartLine className="w-4 h-4" /> },
            { label: "Exclusive features", value: "Included", icon: <FaStar className="w-4 h-4" /> },
            { label: "Early access", value: "Included", icon: <FaBolt className="w-4 h-4" /> }
          ]}
...
function PlanCard({
  ...
  return (
    <div className={`relative bg-[#F5EDD8] rounded-xl p-4 border-2 transition-all duration-300 flex flex-col ${
      isCurrentPlan
        ? 'border-[#D4A84B] shadow-2xl'
        : isPremium
        ...
        {features.map((feature, index) => (
          <div key={index} className="flex items-start gap-2">
            <div className="text-green-600 flex-shrink-0 mt-0.5">
              {feature.icon || <HiCheckCircle className="w-4 h-4" />}
            </div>
